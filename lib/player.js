 ...
 * Catch all route
 */
Router.route('/:var(user|r)?/(.*)', function() {
  this.render('player');
  ...
  Meteor.call('fetchSubreddit', subreddit_link, sid, function(err, data) {
    ...
    if (!data || !data.success) {
      ...
      if (data && data.message) {
        Session.set('displayMessage', data.message);
        if (data.message === 'Reddit servers are busy right now') {
          Session.set('busyReddit', true);
        }
      }
      return;
    }
...
if (Meteor.isClient) {
  ...
  Template.player.events({
    'click #reload-icon': function(event) {
      location.reload();
    },
