...
/**
 * make adjustments to the ui if the user is on mobile
 */
...
Router.route('/(.*)', function() {
  ...
  // register a pageview for current path in google analytics
  var path = Router.current().location.get().path;
  GAnalytics.pageview(path);
  Session.set('path', path);

  // parse url and set footer ui
  if (sort.parseUrl) {
    sort.parseUrl();
  }
  ...
  console.info('fetching from ' + subreddit_link);
  ...
  // get subreddit data from server only if the request url is different
  // than currently loaded posts
  if (old_link !== subreddit_link) {
    ...
    Meteor.call('fetchSubreddit', subreddit_link, sid, function(err, data) {
      ...
      for (var i = 0; i < posts.length; i++) {
        ...
        // check all image thumbnails and see if they return 404
